...
const MAX_GEMINI_INPUT_CHARS = 52000;
    ...
    <main class="app-shell">
      <aside class="sidebar">
        ...
        <section class="panel">
          ...
          <p class="helper-text">GitHub Pages 정적 모드입니다. 파일은 서버로 업로드되지 않고 현재 브라우저에서만 읽습니다. Android 프로젝트는 ZIP으로 첨부할 수 있습니다.</p>
...
function fileStripTemplate() {
  ...
  const displayItems = files.length ? files : sourceMeta;
  return `
    <section class="file-summary" aria-label="첨부파일 목록">
      <div class="section-heading">
        <div>
          <span class="eyebrow">첨부파일</span>
          <h2>문서 작성에 사용할 자료</h2>
        </div>
        <span class="count-badge">${displayItems.length}개</span>
      </div>
      <p class="helper-text">파일명을 확인한 뒤 분석을 실행하세요. 팀 공유 링크에는 첨부파일 원문이 포함되지 않습니다. HWP는 정적 브라우저 제한으로 본문 대신 파일명/메타데이터 근거로 포함됩니다.</p>
      <div class="file-strip">
        ${displayItems.map((file, index) => `
          <div class="file-chip">
            <span class="file-label">${escapeHtml(file.extension || "file")}</span>
            <strong title="${escapeAttr(file.filename)}">${escapeHtml(file.filename)}</strong>
            <span class="${files.length ? (file.parsed_status === "parsed" ? "ok" : "fail") : "muted"}">${escapeHtml(files.length ? file.parsed_status : "excluded")}</span>
            <button
              type="button"
              class="file-remove-btn"
              title="이 파일 제거"
              aria-label="${escapeAttr(file.filename)} 제거"
              ${files.length ? `data-remove-file-index="${index}"` : `data-remove-meta-index="${index}"`}
              ${state.busy ? "disabled" : ""}
            >×</button>
          </div>
  ...
  return `
    <article class="editor">
      <header class="editor-toolbar">
        ...
        <button id="save-draft" class="save-button">저장</button>
      </header>
      <div class="paper">
...
async function analyzeProject() {
  ...
  await withStatus("인허가 문서 초안을 생성하는 중입니다.", async () => {
    ...
    if (state.settings.gemini_api_key) {
      ...
      render();
      try {
        draft = await generateDraftWithGemini(parsedFiles);
      } catch (error) {
        ...
        draft = generateLocalDraft(parsedFiles, error.message);
      }
    } else {
      draft = generateLocalDraft(parsedFiles);
    }
...
async function generateDocumentArchitecture() {
  ...
  await withStatus("문서 전체 아키텍처를 생성하는 중입니다.", async () => {
    ...
    if (state.settings.gemini_api_key) {
      ...
      render();
      try {
        draft = await generateArchitectureWithGemini(parsedFiles);
      } catch (error) {
        ...
        draft = generateLocalArchitectureDraft(parsedFiles, error.message);
      }
    } else {
      draft = generateLocalArchitectureDraft(parsedFiles);
    }
...
async function generateArchitectureWithGemini(files) {
  ...
  const sourceText = files.map(file => `### ${file.filename} (${file.extension}, ${formatBytes(file.size)})\n${file.text}`).join("\n\n---\n\n").slice(0, MAX_GEMINI_INPUT_CHARS);
  const profile = currentDocumentProfile();
  const inventory = buildArtifactInventory(files);
...
function generateLocalArchitectureDraft(files, fallbackReason = "") {
  const profile = currentDocumentProfile();
  ...
  const missingRows = buildMissingInfoRows(profile, files);
  ...
  const blocks = [
    ...
    headingBlock(2, 2, "1. 섹션 설계"),
    tableBlock(3, "문서 섹션 아키텍처", ["순서", "상위 섹션", "목적", "주요 하위 내용", "필요 근거", "작성 상태"], sectionRows),
    headingBlock(4, 2, "2. 첨부파일-섹션 근거 매핑"),
    tableBlock(5, "근거 파일 매핑", ["파일명", "자료 유형", "연결할 문서 섹션", "활용 방식", "비고"], evidenceRows),
    headingBlock(6, 2, "3. 현재 Android/기능 근거 후보"),
    tableBlock(7, "현재 자료 근거 후보", ["구분", "추출 근거", "검토 포인트"], androidRows.length ? androidRows : [["현재 근거", "Android 프로젝트 ZIP 또는 매뉴얼/스크린샷 필요", "[확인 필요: 현재 화면/기능 근거]"]]),
    headingBlock(8, 2, "4. 작성 흐름"),
    { id: makeId("block"), type: "diagram", order: 9, content: { title: "문서 생성 흐름 아키텍처", code: flowCode } },
    headingBlock(10, 2, "5. Claim 관리 표"),
    tableBlock(11, "안전/성능/시험/보안 claim 관리", ["Claim 영역", "현재 표현", "필요 근거", "처리 원칙"], claimRows),
    headingBlock(12, 2, "6. 확인 필요 항목"),
    tableBlock(13, "아키텍처 단계 확인 필요 항목", ["항목", "필요 근거", "우선순위"], missingRows),
    headingBlock(14, 2, "7. 다음 작성 단계"),
    tableBlock(15, "다음 단계", ["순서", "작업", "완료 기준"], [
    ...
    headingBlock(16, 2, "8. 전체 입력 자료 목록"),
    tableBlock(17, "입력 자료 인벤토리", ["파일명", "분류", "형식", "크기", "요약"], inventoryRows),
  ];
  return { title, blocks };
}
...
async function generateDraftWithGemini(files) {
  const model = state.settings.gemini_model || DEFAULT_MODEL;
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(state.settings.gemini_api_key)}`;
  const sourceText = files.map(file => `### ${file.filename} (${file.extension}, ${formatBytes(file.size)})\n${file.text}`).join("\n\n---\n\n").slice(0, MAX_GEMINI_INPUT_CHARS);
  const profile = currentDocumentProfile();
...
function generateGenericLocalDraft(files, fallbackReason = "") {
  ...
  const blocks = [
    ...
    paragraphBlock(1, `${fallbackReason ? `Gemini 생성 실패 사유: ${fallbackReason}\n` : ""}총 ${files.length}개 파일을 브라우저에서 읽어 초안을 생성했습니다.`),
    headingBlock(2, 2, "첨부파일 요약"),
    tableBlock(3, "분석 대상 파일", ["파일명", "크기", "형식", "요약"], rows),
    headingBlock(4, 2, "주요 키워드"),
    tableBlock(5, "반복 출현 키워드", ["키워드", "출현 수"], keywordRows.length ? keywordRows : [["확인 필요", "0"]]),
    headingBlock(6, 2, "추출 텍스트 미리보기"),
    paragraphBlock(7, textPreview.slice(0, 5000)),
  ];
  return { title: state.project.title || "문서 분석 초안", blocks };
}

function generateRegulatoryLocalDraft(files, fallbackReason = "") {
  const profile = currentDocumentProfile();
  const inventoryRows = files.map(file => [file.filename, classifyArtifact(file), file.extension, formatBytes(file.size), file.summary || "요약 없음"]);
  const androidRows = extractAndroidEvidenceRows(files);
  const missingRows = buildMissingInfoRows(profile, files);
  ...
  const blocks = [
    ...
    paragraphBlock(1, `${fallbackReason ? `Gemini 생성 실패 사유: ${fallbackReason}\n` : ""}본 초안은 첨부파일을 브라우저에서 읽어 생성한 인허가 문서 검토용 초안입니다. 규제/안전/성능/시험 결과는 제공된 근거 범위 내에서만 반영하며, 확인되지 않은 항목은 [확인 필요: ...]로 표시했습니다.`),
    headingBlock(2, 2, "1. Intake / 입력 자료 분류"),
    tableBlock(3, "첨부 자료 분류", ["파일명", "분류", "형식", "크기", "요약"], inventoryRows),
    headingBlock(4, 2, "2. 사용자 입력 제약"),
    tableBlock(5, "현재 버전 기본 정보", ["항목", "내용"], [
      ["프로그램명", profile.product_name || "[확인 필요: 현재 프로그램명]"],
      ["SW 버전/빌드", profile.software_version || "[확인 필요: SW 버전/빌드]"],
      ["대상 장비/하드웨어", profile.target_hardware || "[확인 필요: 대상 장비/하드웨어]"],
      ["대상 규제/기관", profile.target_regulator || "[확인 필요: 대상 규제/기관]"],
    ...
    headingBlock(6, 2, "3. Reference Structure"),
    paragraphBlock(7, "기존 승인/참조 문서가 첨부된 경우 섹션 순서, 반복 공식 문구, 안전 주의 문구, 정의, 시험 방법 문구를 유지 대상으로 검토해야 합니다. 기존 문서가 없거나 식별되지 않으면 [확인 필요: prior approved/reference document]로 남겨야 합니다."),
    headingBlock(8, 2, "4. Current Evidence Map"),
    tableBlock(9, "Android/현재 자료 근거 맵", ["구분", "추출 근거", "검토 포인트"], androidRows.length ? androidRows : [["현재 근거", "Android 프로젝트 ZIP 또는 매뉴얼/스크린샷 필요", "[확인 필요: 현재 화면/기능 근거]" ]]),
    headingBlock(10, 2, "5. Change Map"),
    tableBlock(11, "Old vs Current 변경 맵", ["Area", "Prior document", "Current evidence", "Action"], changeRows),
    headingBlock(12, 2, "6. Submission-ready Draft"),
    paragraphBlock(13, `제품명 ${profile.product_name || "[확인 필요: 프로그램명]"}의 현재 버전 소프트웨어는 ${profile.target_hardware || "[확인 필요: 대상 장비]"}와 함께 사용되는 소프트웨어로, ${profile.intended_use || "[확인 필요: 사용 목적]"}을 지원합니다. 본 문안은 첨부 자료 기준 초안이며, 최종 제출 전 기존 승인 문서의 공식 문구와 시험 결과표를 대조해야 합니다.`),
    headingBlock(14, 2, "7. 검토 메모"),
    tableBlock(15, "확인 필요 항목", ["항목", "필요 근거", "우선순위"], missingRows),
    tableBlock(16, "사용한 근거 파일", ["파일명", "근거 유형", "비고"], inventoryRows.map(row => [row[0], row[1], row[4]])),
  ];
...
function normalizeDraft(draft, fallbackTitle) {
  ...
  if (!normalized.blocks.length) {
    normalized.blocks = generateLocalDraft(state.project.files.filter(file => file.parsed_status === "parsed")).blocks;
  }
...
function buildArtifactInventory(files) {
  return files.map(file => `- ${file.filename}: ${classifyArtifact(file)} / ${file.extension} / ${formatBytes(file.size)} / ${file.summary || "요약 없음"}`).join("\n");
}
...
function buildMissingInfoRows(profile, files) {
  const rows = [];
  ...
  if (!files.some(file => classifyArtifact(file) === "Reference regulatory document")) rows.push(["기존 승인/참조 문서", "이전 인허가 문서 DOCX/PDF/HWPX 또는 텍스트 export", "상"]);
